"""
Test video generation functionality for the Route Squiggler render client.
This module handles running test videos from local test folders.
"""
...
class TestVideoManager:
    """Manager class for handling test video generation."""
    ...
    def test_video(self):
        """Show folder selection dialog for 'test videos' directory and process selected folder."""
        try:
            ...
            if not os.path.exists(base_dir):
                ...
                QMessageBox.warning(
                    self.main_window, 
                    "Directory Not Found", 
                    f"The '{base_dir}' directory was not found.\n\nPlease create this directory and add test video folders to it."
                )
                return
            ...
            # Disable button
            self.main_window.test_video_button.setEnabled(False)

            self.start_next_test_video_job()

        except Exception as e:
            self.main_window.log_widget.add_log(f"Failed to initiate test video jobs: {str(e)}")
            self.main_window.log_widget.add_log(traceback.format_exc())

    def start_next_test_video_job(self):
        """Load data for the current test index and use existing workflow."""
        if self.current_test_index >= len(self.test_job_folders):
            # All jobs done
            self.main_window.test_video_button.setEnabled(True)
            self.main_window.test_video_button.setText("Test video")
            self.main_window.log_widget.add_log("All test video jobs completed.")
            self.test_job_folders = []  # Clear the list to exit test mode
            return

        folder = self.test_job_folders[self.current_test_index]
        self.main_window.log_widget.add_log(f"Starting test video job {self.current_test_index + 1}/{len(self.test_job_folders)}: {folder}")
        ...
        try:
            ...
            self.main_window.test_video_button.setText(f"Processing {self.current_test_index + 1}/{len(self.test_job_folders)}")
            ...
            self.main_window.log_widget.add_log(f"Failed to start video job in {folder}: {str(e)}")
            self.main_window.log_widget.add_log(traceback.format_exc())
            # Move to next job on failure
            self.current_test_index += 1
            self.start_next_test_video_job()
